...
class _AdminUserAccountsState extends ConsumerState<AdminUserAccountsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      // Fetch from profiles table — adjust column names if different
      final data = await _sb
          .from('profiles')
          .select('id, full_name, email, role, created_at, avatar_url, phone')
  ...
  Future<void> _setRole(String userId, String newRole) async {
    try {
      await _sb.from('profiles').update({'role': newRole}).eq('id', userId);
      await _load();
      if (mounted) {
    ...
    return Scaffold(
      ...
      body: Column(children: [
        ...
        Container(
          ...
          color: cs.surfaceContainerLowest,
          child: Row(children: [
            _StatChip(cs: cs, label: 'Total', value: _profiles.length.toString(), color: cs.primary),
            const SizedBox(width: 12),
            _StatChip(cs: cs, label: 'Customers', value: _profiles.where((p) => p['role'] == 'customer').length.toString(), color: cs.secondary),
            const SizedBox(width: 12),
            _StatChip(cs: cs, label: 'Partners', value: _profiles.where((p) => p['role'] == 'partner').length.toString(), color: Colors.orange),
            const SizedBox(width: 12),
            _StatChip(cs: cs, label: 'Admins', value: _profiles.where((p) => ['admin','sysadmin'].contains(p['role'])).length.toString(), color: AppColors.fireRed),
            const Spacer(),
            Text('Showing ${_filtered.length} of ${_profiles.length}', style: TextStyle(fontSize: 11, color: cs.onSurfaceVariant)),
          ]),
        ),
        // List
      ...
      child: Row(children: [
        ...
        Container(
          ...
          child: avatarUrl != null && avatarUrl.isNotEmpty
              ? Image.network(avatarUrl, fit: BoxFit.cover, errorBuilder: (_, __, ___) => Center(child: Text(initials, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 14))))
        ...
        Container(
          ...
          decoration: BoxDecoration(color: roleColor.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(12), border: Border.all(color: roleColor.withValues(alpha: 0.3))),
